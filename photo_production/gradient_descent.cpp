...
//		};
	...
	std::vector<Point2D> sample_points(data_points.begin() + 59, data_points.begin()+260);

	auto rho = M_scattering.scattering();
	std::cout << "{";
	for( auto& p : data_points ){
		std::cout << "{" << p.x << "," << rho(p.x).real() * static_cast<double>(1._2mbarn) << "},";
	}
	std::cout << "\b};\n";

	auto d1232 = scattering_amplitudes[0].scattering();
	std::cout << "{";
	for( auto& p : data_points ){
		std::cout << "{" << p.x << "," << d1232(p.x).real() * static_cast<double>(1._2mbarn) << "},";
	}
	std::cout << "\b};\n";

//	std::cout << "sample_points: " << sample_points.size() << "\n";
...
//	Amplitude<1> interference = scattering_amplitudes[0];
	...
	for( std::size_t epoch = 0; epoch < N_EPOCHS; ++epoch ){
		...
		for( auto const& point : sample_points ){
			double diff = point.y - y_hat(point.x).real() * static_cast<double>(1._2mbarn);
//			std::cout << FORMAT("diff: {} {} {} {}", point.x, point.y, y_hat(point.x).real(), diff) << "\n";
			loss += diff*diff;
			for( std::size_t k = 1; k < N_amplitudes-1; ++k ){
//				std::cout << "k: " << k << " ";
//				std::cout << "loss: " << diff*diff << " ";
				double const loss0 = std::pow(point.y - y_hat0[k](point.x).real() * static_cast<double>(1._2mbarn), 2);
//				std::cout << "loss0: " << loss0 << " ";
				double const loss1 = std::pow(point.y - y_hat1[k](point.x).real() * static_cast<double>(1._2mbarn), 2);
...
//	}
	...
	for( auto& point : data_points ){
		std::cout << "{" << point.x << "," << result(point.x).real() * static_cast<double>(1._2mbarn) << "},";
	}
	std::cout << "\b};\n\n";
